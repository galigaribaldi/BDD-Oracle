...
begin
    ...
    insert into ts_auto_f3(auto_id,foto) select auto_id,foto
    from auto_f3 where auto_id = p_auto_id;
...
create or replace function get_remote_r_pago_f1_by_id(p_cliente_id in number, p_auto_id in number) 
    return blob is
    pragma autonomous_transaction;
    v_temp_pdf blob;
begin
    ...
    where auto_id = p_auto_id and cliente_id = p_cliente_id;
    --elimina los registros de la tabla temporal una vez que han sido obtenidos.
    delete from ts_pago_auto_f1;
    -----------------------------------------------------------
    commit;
...
create or replace function get_remote_r_pago_f2_by_id(p_cliente_id in number, p_auto_id in number) 
    return blob is
    pragma autonomous_transaction;
    v_temp_pdf blob;
begin
